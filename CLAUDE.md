...
packages/
  ...
  skill/     — Downloadable AI agent skill (SKILL.md), regenerated by the docs build
```
...
- **`packages/skill/SKILL.md`** — generated from `packages/docs/src/pages/skill.ts`. Injects the component list and CDN URL from source. Edit the template in `skill.ts` instead.
